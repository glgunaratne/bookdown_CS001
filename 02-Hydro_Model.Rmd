...
These data were provided by UWA as they sourced these from [ROMS](https://www.myroms.org/). It is a free-surface, primitive equations ocean model widely used by the scientific community for a diverse range of applications [@Haidvogel2000]. ROMS is used by UWA researchers through the whole state: from the Kimberley region to the north extending into south‐Australia. Within this domain there are a number of applications focussing on specific regions and processes [@Middleton2012].
...
It has a generalized vertical, terrain-following, coordinate system. In addition to that, horizontal velocity components, salinity and temperatures are computed on separate grids (\@ref(fig:stv-grid)). ROMS data were post-processed to make it compatible with the TUFLOFV model mesh.
